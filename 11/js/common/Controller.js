...
Controller.prototype.PageContentVisiblityFun = function () {
	if (model.pageLoaded) {
		...
		if (model.subPageInnerFlg) {
			...
			// NEW CODE
			$("#pagTitle").attr("style", "display: inline-block !important");
			$("#moduleTitle").attr("style", "display: inline-block !important");
			$("#tpcTitle").attr("style", "display: inline-block !important");
			$("#tpcTitle").html(model.PageTitleArr[model.currTopic][model.currMod][model.currPage])
			$("#moduleTitle").html(model.SubPageTitleLevel1Arr[model.currTopic][model.currMod][model.currPage][model.subcurrPage])
			$("#pagTitle").html(model.SubPageTitleLevel2Arr[model.currTopic][model.currMod][model.currPage][model.subcurrPage][model.subInnercurrPage])
		} else if (model.subPageFlg) {
			console.log('subpageflg');
			// OLD CODE
			...
			//$("#moduleTitle").attr("style", "display: none !important");
			$("#tpcTitle").attr("style", "display: none !important");
			if (model.PageTitleArr[model.currTopic][model.currMod].length > 1) {
				console.log('len is more than 1');
				$("#moduleTitle").attr("style", "display: none !important");
				$("#pagTitle").attr("style", "display: inline-block !important");
			} else {
				console.log('len is less than 1');
				$("#moduleTitle").attr("style", "display: inline-block !important");
				$("#moduleTitle").html(model.PageTitleArr[model.currTopic][model.currMod]);
				$("#pagTitle").attr("style", "display: none !important");
			}
	...
	if (!model.ContentVisibleFlg) {
		...
		setTimeout(function () {
			$("#txt_open_close").addClass("txt_close");
		}, 1000);
...
Controller.prototype.updateViewNow = function () {
	...
	tempTotalPage = model.totalPages
	...
	if (model.currMod == (model.tModulesArr[model.currTopic])) {
...
Controller.prototype.pageVisitedCalcFun = function () {
	...
	//RK 03/22  this for this module only
	var rkFg = true;
	for(var k =0; k< model.tTopics; k++)
	for (var t = 0; t < model.tTopics; t++) {
		for (var s = 0; s < model.tModulesArr[k]; s++) {
		var nodeTopic=$("#shell_mod_" + s +"_title")
				
			if(!nodeTopic.hasClass("icon-fullyCompleted"))
			{
			rkFg = false;	
			}
